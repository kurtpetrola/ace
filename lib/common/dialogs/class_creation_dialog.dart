// lib/common/dialogs/class_creation_dialog.dart
...
class ClassCreationDialog extends StatefulWidget {
  ...
  // If provided, the class will be automatically assigned to this teacher
  // and the teacher selection dropdown will be hidden.
  final String? teacherId;

  const ClassCreationDialog({
    super.key,
...
class _ClassCreationDialogState extends State<ClassCreationDialog> {
  ...
  void initState() {
    ...
    if (widget.teacherId != null) {
      // TEACHER MODE: Auto-assign
      _selectedTeacherId = widget.teacherId;
      _fetchCurrentTeacherName();
    } else {
      // ADMIN MODE: Fetch all teachers
      _fetchTeachers();
    }
  }

  Future<void> _fetchCurrentTeacherName() async {
    // We need the teacher's name for the 'creator' field
    // In a real app, you might pass the full User object or fetch it.
    // Here we'll just fetch it to be safe.
    final user = await _userService.getTeacher(widget.teacherId!);
    if (mounted) {
      setState(() {
        _creator = user?.fullname ?? 'Teacher';
        _isLoading = false;
      });
    }
  }
              ...
              child: SingleChildScrollView(
                child: Column(
                  ...
                  children: <Widget>[
                    ...
                    // TEACHER SELECTION (Only in Admin Mode)
                    if (widget.teacherId == null) ...[
                    ...
                    DropdownButtonFormField<String>(
                      ...
                      items: _mockBannerPaths.map((String path) {
                        return DropdownMenuItem<String>(
                          value: path,
                          child: Text(
                              path.split('/').last), // Show just the filename
                        );
                      }).toList(),
